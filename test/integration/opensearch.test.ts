/**
 * Integration tests for the OpenSearch adapter, against a real container.
 *
 * Gated behind RUN_INTEGRATION=1 (set by `npm run test:integration`). Start the
 * engine first: `docker compose --profile os up -d` (it listens on host 9201).
 */
...
const NODE = process.env.OS_NODE ?? 'http://localhost:9201';
...
describe.skipIf(!RUN)('OpenSearchAdapter (integration)', () => {
  const engine = new OpenSearchAdapter({ node: NODE });
